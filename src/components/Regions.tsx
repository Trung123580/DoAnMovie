...
import { getRegionsMovie } from '@/service';
...
function page({ slug }: { slug: string }) {
  ...
  const categoryName = regions.find((item: any) => item.slug === slug)?.name;
  ...
  useEffect(() => {
    (async () => {
      ...
      const response = await getRegionsMovie(slug, Number(searchPage));
